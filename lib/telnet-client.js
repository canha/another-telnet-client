(function () {
    ...
    Telnet.prototype.connect = function (opts) {
        ...
        this.failedLoginPrompt = (typeof opts.failedLoginPrompt !== 'undefined' ? opts.failedLoginPrompt : undefined);
        this.response = '';
    ...
    function parseData(chunk, telnetObj) {
        ...
        if (telnetObj.telnetState === 'getprompt') {
            ...
            promptIndex = tempStringData.search(regexEscape(telnetObj.shellPrompt));
            ...
            else if (tempStringData.search(regexEscape(telnetObj.loginPrompt)) !== -1) {
                telnetObj.telnetState = 'login';
                login(telnetObj, 'username');
            }
            else if (tempStringData.search(regexEscape(telnetObj.passwordPrompt)) !== -1) {
                telnetObj.telnetState = 'login';
            ...
            if (tempStringData.search(regexEscape(telnetObj.enablePrompt)) !== -1) {
                telnetObj.telnetState = 'login';
            ...
            if (tempStringData.search(regexEscape(telnetObj.shellPrompt)) !== -1) {
                telnetObj.telnetState = 'login';
            ...
            if (promptIndex === -1 && tempStringData.length !== 0) {
                if (tempStringData.search(regexEscape(telnetObj.pageSeparator)) !== -1) {
    ...
    function login(telnetObj, handle) {
        if (handle === 'username' && telnetObj.telnetSocket.writable) {
            telnetObj.telnetSocket.write(telnetObj.username + telnetObj.ors, function () {
                telnetObj.telnetState = 'getprompt';
            });
        }
        else if (handle === 'password' && telnetObj.telnetSocket.writable) {
        ...
        else if (handle === 'enable' && telnetObj.telnetSocket.writable) {
            telnetObj.telnetSocket.write("en" + telnetObj.ors, function () {
                telnetObj.telnetState = 'enable';
            });
        }
        else if (handle === 'enablePassword' && telnetObj.telnetSocket.writable) {
